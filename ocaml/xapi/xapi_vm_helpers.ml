...
let assert_gpus_available ~__context ~self ~host =
  ...
  let vgpurs = List.map (Vgpuops.vgpu_of_ref ~__context)  vgpus in
    try
        ignore (List.fold_left (fun allocate_list vgpu -> Vgpuops.allocate_vgpu_to_gpu ~dry_run:true ~pre_allocate_list:allocate_list ~__context self host vgpu) [] vgpurs) 
